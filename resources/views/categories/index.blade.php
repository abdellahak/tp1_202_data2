...
<body class="bg-slate-900 text-white dark:bg-white dark:text-black relative min-h-screen">
  <div class="flex min-h-screen">
    <div
      class="container hidden md:block w-fit min-w-56 min-h-full bg-slate-800 text-white dark:bg-gray-200 dark:text-black">
        ...
        <ul>
          ...
          <li class="my-1">
            <a href="{{ route('categories.index') }}"
              class="w-full hover:bg-slate-700 bg-slate-700 dark:bg-gray-300 block p-3 text-md border-r-4 border-r-solid border-r-blue-600"><i
                class="fa-solid fa-code-fork mx-2"></i>Categories</a>
          </li>
          <li class="my-1">
            <a href="{{ route('produits.index') }}"
              class="w-full hover:bg-slate-700 dark:bg-gray-300 block p-3 text-md"><i
    ...
    <div class="w-full mx-auto bg-slate-900 dark:bg-gray-100 flex flex-col h-screen">
      <div class="flex justify-between items-center mb-4 p-4 dark:bg-white bg-gray-800 w-full">
        ...
        <div class="flex flex-col gap-2 md:flex-row">
          <a href="{{ route('categories.create') }}"
            class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700 block">Ajouter Categorie</a>
      ...
      <div class="container flex flex-col md:flex-row gap-2 p-4 pt-0">
        <div class="flex flex-row items-center">
          <p class="bg-slate-800 dark:bg-gray-200 w-full md:w-fit px-2 py-2 rounded-l-lg ">Le nombre des categories
            est
            : </p>
          <p
            ...
            @foreach ($categories as $item)
              <tr class="hover:bg-slate-700 dark:hover:bg-gray-300" id="item-{{ $item->id }}">
                ...
                <td class="py-2 px-4 text-center">
                  <a href="{{ route('categories.show', $item->id) }}" class="text-blue-500 hover:underline"
                    title="Details">
                    <i class="fa-solid fa-circle-info"></i>
                  </a>
                </td>
                <td class="py-2 px-4 text-center">
                  <a href="{{ route('categories.edit', $item->id) }}" class="text-yellow-500 hover:underline"
                    title="Modifier">
