    ...
    <div class="flex min-h-full bg-gray-100">
      ...
      <div class="mx-auto w-full bg-slate-900 dark:bg-gray-100">
        ...
        <div class="max-w-2xl mx-auto p-6 dark:bg-white bg-slate-800  shadow-md rounded-lg">
          <h1 class="text-2xl font-bold mb-6">Modifier le produit ayant l'id N° {{ $produit->id }}</h1>
          @if ($errors->any())
          ...
          <form action="{{-- route('produits.update', $produit->id) --}}" method="POST">
            ...
            <div class="mb-4">
              <label for="prix" class="block dark:text-gray-700 text-gray-100 mb-2 text-lg ">Prix :</label>
              <input type="number" id="prix" name="prix" value="{{ old('prix', $produit->prix) }}"
                class="w-full my-2 px-2 py-2 rounded focus:outline-none dark:text-black text-white bg-slate-700 dark:bg-gray-200 ">
            </div>
            <div class="mb-4">
              <label for="quantite" class="block dark:text-gray-700 text-gray-100 mb-2 text-lg">Quantité :</label>
              <input type="number" id="quantite" name="quantite" value="{{ old('quantite', $produit->quantite) }}"
                class="w-full my-2 px-2 py-2 rounded focus:outline-none dark:text-black text-white bg-slate-700 dark:bg-gray-200 ">
            </div>
            <div>
              <label for="categorie_id" class="block dark:text-gray-700 text-gray-100 mb-2 text-lg">Catégorie :</label>
              <select name="categorie_id" id="categorie_id" class="w-full my-2 px-2 py-2 rounded focus:outline-none dark:text-black text-white bg-slate-700 dark:bg-gray-200 ">
                  @foreach ($categories as $item)
                      <option value="{{$item->id}}" {{$item->id == ($produit->categorie->id ?? '') ? 'selected' : ''}}>{{$item->nom}}</option>
                  @endforeach
              </select>
            </div>
            <div class="mb-4">
              ...
              <textarea name="description" id="description" cols="30" rows="3"
                class="w-full my-2 px-2 py-2 rounded focus:outline-none dark:text-black text-white bg-slate-700 dark:bg-gray-200 ">{{ old('description', $produit->description) }}</textarea>
            </div>
